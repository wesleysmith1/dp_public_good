...
import csv
import math
from mechanisms.helpers import write_session_dir
...
def generate_payouts(group):
    ...
    # pymt_round = random.randint(2,19)
    ...
    for player in group.get_players():
        ...
        if 'balances' in player.participant.vars.keys() and len(participant_balances) > 1:
            ...
            balance = grain_to_dollars(group, sum(participant_balances))
...
def generate_survey_csv(group):
    ...
    session_date = group.session.vars.get('session_date', 999)
